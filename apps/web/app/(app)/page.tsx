...
import { getPendingToLog, getUpcomingEvents, searchEvents } from '@/lib/queries/events';
import { getFeed, getViewerStats, getViewerTopSports } from '@/lib/queries/logs';
...
import { CompactFixtureCalendar } from '@/components/fixture/CompactFixtureCalendar';
import { ArchiveStatsPanel } from '@/components/home/ArchiveStatsPanel';
import { PendingToLogPanel } from '@/components/home/PendingToLogPanel';
...
export default async function HomePage() {
  ...
  const sportSlugs = topSports.length ? topSports.map((s) => s.slug) : undefined;

  const [recentFeed, trending, upcoming, competitions, featuredLists, viewerStats, pendingToLog] = await Promise.all(
    [
      ...
      // MYSPORTS-scoped calendar as the prototype, all sports for a
      // logged-out visitor or a viewer with no logs yet.
      getUpcomingEvents({ sportSlugs, limit: 12 }),
      searchCompetitions({ limit: 4 }),
      getFeaturedLists(6),
      viewerId ? getViewerStats(viewerId) : Promise.resolve(null),
      viewerId ? getPendingToLog({ viewerId, sportSlugs, limit: 3 }) : Promise.resolve([]),
    ],
  );
